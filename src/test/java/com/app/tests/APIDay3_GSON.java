...
public class APIDay3_GSON {
	...
	Response response =
		...
		.when().get(ConfigurationReader.getProperty("hrapp.basedresturl")+"/employees/120");
	
	// DE-SERIALIZATION: JSON -> JAVA (Collections)
	...
	public void convertJsonToListofMaps() {
		Response response =
				 given().accept(ContentType.JSON)
				.when().get(ConfigurationReader.getProperty("hrapp.basedresturl")+"/departments");
		
		// convert the response that contains department information int list of maps 
